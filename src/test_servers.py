...
def test_response_ok_returns_response():
    ...
    assert b"200" in response_ok("GET /path/to/index.html HTTP/1.1\r\nHost: www.mysite1.com:80\r\n\r\n")


def test_output_type_of_response_ok():
    """Test that the output is the correct type."""
    from server import response_ok
    assert isinstance(response_ok("GET /path/to/index.html HTTP/1.1\r\nHost: www.mysite1.com:80\r\n\r\n"), bytes)


def test_output_of_response_ok_exists():
    """Test that the output exists."""
    from server import response_ok
    assert response_ok("GET /path/to/index.html HTTP/1.1\r\nHost: www.mysite1.com:80\r\n\r\n") is not None
...
def test_output_type_of_resolve_uri():
    ...
    assert type(resolve_uri('src/test.txt')) == tuple
